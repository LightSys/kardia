...
fundmgr_report "widget/page"
	...
	vb2 "widget/vbox"
		...
		field='manager_id';
		text='Fund Manager:';
		popup_sql=runserver("select value = :p:p_partner_key, label = :p:p_partner_key + ' ' + condition(char_length(rtrim(:p:p_org_name)) > 0, :p:p_org_name, :p:p_given_name + ' ' + :p:p_surname) + ' (' + count(1) + ' fund' + condition(count(1) == 1, '', 's') + ')', fundcnt=count(1) from /apps/kardia/data/Kardia_DB/p_partner/rows p, /apps/kardia/data/Kardia_DB/p_staff/rows ps, /apps/kardia/data/Kardia_DB/a_fund_staff/rows cs, /apps/kardia/data/Kardia_DB/a_fund/rows c where :p:p_partner_key = :ps:p_partner_key and :ps:p_is_staff = 1 and :cs:a_ledger_number = " + quote(:this:ledger) + " and :cs:p_staff_partner_key = :ps:p_partner_key and :c:a_ledger_number = :cs:a_ledger_number and :c:a_fund = :cs:a_fund and :c:a_fund_class != 'MIS' group by :p:p_partner_key having :fundcnt > 0");
		//popup_sql=runserver("select value = :p:p_partner_key, label = condition(char_length(rtrim(:p:p_org_name)) > 0, :p:p_org_name, :p:p_given_name + ' ' + :p:p_surname) + ' #' + :p:p_partner_key, fundcnt=(select count(1) from /apps/kardia/data/Kardia_DB/a_fund_staff/rows cs, /apps/kardia/data/Kardia_DB/a_fund/rows c where :cs:a_ledger_number = " + quote(:this:ledger) + " and :cs:p_staff_partner_key = :p:p_partner_key and :c:a_ledger_number = :cs:a_ledger_number and :c:a_fund = :cs:a_fund and :c:a_fund_class != 'MIS')  from /apps/kardia/data/Kardia_DB/p_partner/rows p, /apps/kardia/data/Kardia_DB/p_staff/rows ps where :p:p_partner_key = :ps:p_partner_key and :ps:p_is_staff = 1 having :fundcnt > 0");
