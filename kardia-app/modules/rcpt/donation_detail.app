$Version=2$
donation_detail "widget/page"
    {
    title = "i18n:Cash Receipts Detail";
    width=580;
    height=525;
	...
	vb2 "widget/vbox"
	    ...
	    spacing=4;
	    lbl_tb "widget/label" { height=30; font_size=16; text="i18n:Cash Receipts Detail Options:"; align=center; }
		...
		year_sel_cn "widget/connector"
		    ...
		    //event_confirm=runclient("select :a_period + ' - ' + :a_period_desc, :a_period, :a_summary_only from subtree /apps/kardia/modules/gl/periods.qyt/" + :donation_detail:ledger + "/" + :Value + "|" + :donation_detail:ledger + " order by :a_period asc having :a_summary_only = 0");
		    target=f_start;
		    //action=SetSQL;
		    action=SetGroup;
		    Group = runclient(:Value);
		    //sql = runclient("select :a_period + ' - ' + :a_period_desc, :a_period, :a_summary_only from subtree /apps/kardia/modules/gl/periods.qyt/" + :donation_detail:ledger + "/" + :Value + "|" + :donation_detail:ledger + " order by :a_period asc having :a_summary_only = 0");
		    //sql = runclient("select :a_period + ' - ' + :a_period_desc, :a_period, :a_summary_only from subtree /apps/kardia/modules/gl/periods.qyt/" + :donation_detail:ledger + "/" + :rpt_form:year_period + "|" + :donation_detail:ledger + " order by :a_period asc having :a_summary_only = 0");
		    }
		}
		...
		ref_sel_cn "widget/connector"
		    ...
		    Min = runclient(:Value);
		    //action=SetSQL;
		    //sql = runclient("select :a_period + ' - ' + :a_period_desc, :a_period, :a_summary_only from subtree /apps/kardia/modules/gl/periods.qyt/" + :donation_detail:ledger + "/" + :rpt_form:year_period + "|" + :donation_detail:ledger + " order by :a_period asc having :a_summary_only = 0 and :a_period >= " + quote(:rpt_form:start_period));
		    }
		}
	    ...
	    f_line "widget/component" { x=10; width=400; height=24; path="/sys/cmp/smart_field.cmp"; field="show_line_items"; ctl_type='checkboxleft'; text="i18n:Show Designations"; form=rpt_form; label_width=120; }
	    ...
	    sep2 "widget/autolayoutspacer" { height=172; }
		...
		rpt_cancel "widget/textbutton"
		    ...
		    width=90;
		    text="i18n:Cancel";
		    rpt_cancel_cn "widget/connector" { event="Click"; target="donation_detail"; action="Close"; }
		    }
		}
	    }
	}
