...
function startDialog() {
	...
	for (var i=0;i<window.arguments[0].length;i+=2) {
		document.getElementById("select-tag-type").innerHTML += '<menuitem label="' + window.arguments[0][i+1] + '" value="' + window.arguments[0][i] + '"/>';
	}
...
function saveTag(){
	...
	window.arguments[1].tagId = parseInt(document.getElementById("outer-select-tag").selectedItem.value);
