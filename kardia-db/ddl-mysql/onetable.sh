...
grep -m1 -A100 "^create table ${TABLE} ($" tables_create.sql | grep -B100 -m1 '^\/' | head -n -1 >> onetable.sql

grep -m2 -A10 "^alter table ${TABLE}$" keys_create.sql | grep -B10 -m1 '^$' | head -n -1 >> onetable.sql
