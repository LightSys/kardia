...
grep -m1 -A0 "^drop table if exists ${TABLE};$" tables_drop.sql >> onetable.sql
(grep -m1 -A100 "^create table ${TABLE} ($" tables_create.sql; echo '/') | grep -v '^insert' | grep -B100 -m1 '^\/' | head -n -1 >> onetable.sql
...
(grep -m1 -A100 "^create table ${TABLE} ($" tables_create.sql; echo '/') | grep -B100 -m1 '^\/' | head -n -1 | grep '^insert' >> onetable.sql
