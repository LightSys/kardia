$Version=2$
a_subtrx_deposit "application/filespec"
    {
    // General parameters.
    filetype = csv;
    header_row = yes;
    header_has_titles = no;
    two_quote_escape = yes;
    annotation = "CSV Data for a_subtrx_deposit";
    key_is_rowid = yes;
    new_row_padding = 8;
    
    // Column specifications.
    a_ledger_number "filespec/column" { type=string; id=1; }
    a_batch_number "filespec/column" { type=integer; id=2; }
    a_period "filespec/column" { type=string; id=3; }
    a_effective_date "filespec/column" { type=datetime; id=4; }
    a_from_account_code "filespec/column" { type=string; id=5; }
    a_account_code "filespec/column" { type=string; id=6; }
    a_amount "filespec/column" { type=money; id=7; }
    a_cash_amount "filespec/column" { type=money; id=8; }
    a_num_checks "filespec/column" { type=integer; id=9; }
    a_posted "filespec/column" { type=integer; id=10; }
    a_posted_to_gl "filespec/column" { type=integer; id=11; }
    a_comment "filespec/column" { type=string; id=12; }
    s_date_created "filespec/column" { type=datetime; id=13; }
    s_created_by "filespec/column" { type=string; id=14; }
    s_date_modified "filespec/column" { type=datetime; id=15; }
    s_modified_by "filespec/column" { type=string; id=16; }
    __cx_osml_control "filespec/column" { type=string; id=17; }
    }
